...
const FlowContent = () => {
  ...
  // Sincronização automática apenas para mudanças nos elementos (nodes/edges) e se estiver em modo de edição
  useEffect(() => {
    if (!modoEdicao || (nodes.length === 0 && edges.length === 0)) return;
    const timer = setTimeout(salvarNoSupabase, 3000);
    return () => clearTimeout(timer);
  }, [nodes, edges, modoEdicao, salvarNoSupabase]);
  ...
  const exportarPDF = useCallback(async () => {
    ...
    const styleId = 'pdf-export-force-styles';
    ...
    styleEl.innerHTML = `
      .react-flow__background, .react-flow__controls, .react-flow__attribution, .search-input-container, .react-flow__panel, .debug-log-container {
        display: none !important;
      }
      .react-flow__node { opacity: 1 !important; visibility: visible !important; }
    ...
    try {
      ...
      // Restaurar espessura das linhas
      originalEdgeStyles.forEach(item => {
        if (item.strokeWidth) item.el.setAttribute('stroke-width', item.strokeWidth);
        else item.el.removeAttribute('stroke-width');
      ...
      // Área disponível (Margens de 10mm)
      const margin = 10;
      const topOffset = 25;
      const availableW = pageWidth - (margin * 2);
      const availableH = pageHeight - margin - topOffset;
      ...
      // Centralização final na área disponível do PDF
      const x = margin + (availableW - imgW) / 2;
      const y = topOffset + (availableH - imgH) / 2;
  ...
  const updateBulkNodes = useCallback((changes: any, append = false) => {
    setNodes(nds => nds.map(n => {
      if (n.selected) {
        ...
        if (changes.uc !== undefined) {
          newNode.data = { ...newNode.data, uc: changes.uc };
        }
        if (changes.medidor !== undefined) {
          newNode.data = { ...newNode.data, medidor: changes.medidor };
        }
        return newNode;
      }
      return n;
  ...
  const alinharVertical = useCallback(() => {
    ...
    // Alinhamento no eixo X (média dos centros horizontais para alinhar os handles superior/inferior)
    const avgCenterX = selectedNodes.reduce((acc, n) => {
      const w = (n as any).width || (n as any).measured?.width || 210;
      return acc + n.position.x + w / 2;
    }, 0) / selectedNodes.length;
    
    // Distribuição no eixo Y (com espaçamento fixo de 25px entre os nós)
    ...
    sortedNodes.forEach((n) => {
      ...
      newPositions.set(n.id, { x: avgCenterX - w / 2, y: currentY });
      
      // Prepara o Y para o próximo nó
  ...
  const onNodeClick = useCallback((event: any, node: any) => {
    ...
    // Se não estiver usando Ctrl, centraliza o nó e desmarca arestas manualmente para limpar o painel
    if (!event.ctrlKey) {
      centralizarNoPalcoUtil([node]);
      setEdges((eds) => eds.map((e) => ({ ...e, selected: false })));
    }
  ...
  const onEdgeClick = useCallback((event: any, _edge: any) => {
    // Se não estiver usando Ctrl, desmarca nós manualmente
    if (!event.ctrlKey) {
      setNodes((nds) => nds.map((n) => ({ ...n, selected: false })));
    }
  ...
  const onPaneClick = useCallback((event: React.MouseEvent) => {
    // Se estiver segurando Ctrl, não limpa a seleção para permitir drag-box ou cliques múltiplos
    ...
    <div style={{ width: '100vw', height: '100vh', display: 'flex', overflow: 'hidden', flexDirection: isMobileView ? 'column' : 'row' }}>
      ...
      {(!isMobileView || showMobileMenu) && (
        ...
        >
          ...
          >
            ...
            {!isMobileView && (
              <div style={{ padding: '12px', borderTop: '1px solid #f1f5f9', background: '#1e293b', color: '#94a3b8', fontSize: '9px', fontFamily: 'monospace', height: '500px', flexShrink: 0, marginBottom: '30px', overflowY: 'auto' }}>
      ...
      <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
            ...
            {modoEdicao ? (
              <input value={projetoAtivo?.nome || ''} 
                onChange={(e) => {
                  ...
                  setProjetos(prev => prev.map(p => p.id === projetoAtivoId ? {...p, nome: val} : p));
                  
                  requestAnimationFrame(() => {
                    input.setSelectionRange(start, end);
                  });
                }} 
                style={{ ...inputHeader, textAlign: 'center', width: '100%', maxWidth: '400px', textTransform: 'uppercase' }} />
            ) : (
              <h1 style={{ 
        ...
        <div style={{ flexGrow: 1, position: 'relative' }}>
            ...
            nodesDraggable={modoEdicao && !ctrlPressed} nodesConnectable={modoEdicao}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {selecionado.nodeCount > 1 && (
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '15px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  ...
                  <div>
                    <label style={labelSmall}>Concessionária em Massa</label>
                    <input type="text" style={inputStyle} placeholder="Alterar todas as concessionárias para..." disabled={!modoEdicao} onChange={(e) => updateBulkNodes({ concessionaria: e.target.value })} />
                  </div>

                  <div>
                    <label style={labelSmall}>UC em Massa</label>
                    <input type="text" style={inputStyle} placeholder="Alterar todas as UCs para..." disabled={!modoEdicao} onChange={(e) => updateBulkNodes({ uc: e.target.value })} />
                  </div>

                  <div>
                    <label style={labelSmall}>Medidor em Massa</label>
                    <input type="text" style={inputStyle} placeholder="Alterar todos os medidores para..." disabled={!modoEdicao} onChange={(e) => updateBulkNodes({ medidor: e.target.value })} />
                  </div>
            ...
            <div>
              ...
              <input 
                ...
                style={{
                  ...inputStyle, 
                  background: 'white', 
                  border: erroModal ? '1px solid #ef4444' : '1px solid #e2e8f0',
                  textTransform: 'uppercase'
                }} 
